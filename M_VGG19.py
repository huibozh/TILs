# -*- coding: utf-8 -*-
"""
Created on Sun Oct  2 21:03:28 2022
...
#from keras.layers import Dense, Flatten, GlobalAveragePooling2D#, Dropout, Activation, Flatten
...
vgg19_model = Model(inputs = vgg.input, outputs = FC_Head)
vgg19_model.summary()
...
vgg19_model.compile(optimizer='adam', loss = "sparse_categorical_crossentropy",metrics = ['accuracy'])
checkpointer = tf.keras.callbacks.ModelCheckpoint('vgg19_model_for_classification.h5', verbose=1, save_best_only=True)
callbacks = [
...
plt.savefig('./plot/VGG19_accuracy.pdf', dpi = 1000)
plt.figure()
...
plt.legend()
plt.savefig('./plot/VGG19_loss.pdf', dpi = 1000)
plt.show()
...
####X_train
probas=vgg19_model.predict(X_train)
...
plt.title("VGG19_confusion_matrix",x=0.5,y=-0.12)
plt.savefig("./plot/VGG19_T_confusion_matrix.pdf", tight_layout=False)
plt.show()
...
plt.title("VGG19_confusion_matrix_errors",x=0.5,y=-0.12)
plt.subplots_adjust(left=0.1, right=0.2, top=0.2, bottom=0.1)
plt.savefig("./plot/VGG19_T_confusion_matrix_errors.pdf", tight_layout=False)
plt.show()


####X_val
probas=vgg19_model.predict(X_val)
...
plt.matshow(conf_mx, cmap=plt.cm.gray)
plt.title("VGG19_confusion_matrix",x=0.5,y=-0.12)
plt.savefig("./plot/VGG19_V_confusion_matrix.pdf", tight_layout=False)
plt.show()
...
plt.title("VGG19_confusion_matrix_errors",x=0.5,y=-0.12)
plt.subplots_adjust(left=0.1, right=0.2, top=0.2, bottom=0.1)
plt.savefig("./plot/VGG19_V_confusion_matrix_errors.pdf", tight_layout=False)
plt.show()